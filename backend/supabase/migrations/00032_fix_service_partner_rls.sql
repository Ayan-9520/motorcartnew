-- Repair script: safe to re-run if 00031 failed partway or only RLS block was executed.
-- Creates missing tables, fixes function order, reapplies all policies.

-- Ensure core dependency exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'service_centers'
  ) THEN
    RAISE EXCEPTION 'Missing public.service_centers — run base Motorcart migrations (00001) first.';
  END IF;
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Missing public.is_admin() — run migration 00009_rbac_profiles_status_rls.sql first.';
  END IF;
END $$;

-- Minimal table bootstrap (IF NOT EXISTS — full DDL in 00031)
...
AS $$
  SELECT sc.id FROM public.service_centers sc WHERE sc.owner_id = auth.uid()
  UNION
  SELECT ss.service_center_id FROM public.service_staff ss
  WHERE ss.user_id = auth.uid() AND ss.is_active = TRUE;
$$;

GRANT EXECUTE ON FUNCTION public.sc_owner_center_ids() TO authenticated;

-- Re-apply RLS (same as 00031 tail)
