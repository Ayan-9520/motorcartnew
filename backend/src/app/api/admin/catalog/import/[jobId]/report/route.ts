...
export async function GET(req: NextRequest, context: Ctx) {
  ...
  try {
    ...
    const record = catalogImportAdminService.getJob(jobId);
    if (!record) return err("Import job not found", 404);

    const report = catalogImportAdminService.getReport(jobId);
    if (!report) {
      return err("Import report not ready — job is still running", 409);
    }

    const preview = catalogImportAdminService.getPreview(jobId, actor.role);
    return ok({ report, preview, dryRun: true as const });
  } catch (e) {
    if (e instanceof ZodError) {
      return err("Invalid job id", 400);
    }
    const msg = e instanceof Error ? e.message : "Failed to load import report";
